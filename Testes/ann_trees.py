...
if __name__ == '__main__':
    ...
    while True:
        ...
        y_pred = (y_pred > 0.5)
        y_pred = [ 1 if x == True else 0 for x in np.array(y_pred.flatten()) ]
        y_pred = np.array(y_pred).reshape(-1, 1)
        ann_y_pred = y_pred

        training = list(np.c_[X_train, y_train])  # Concatena os dados com seus resultados
        evaluation = list(np.c_[X_test, y_test])        # Concatena os dados com seus resultados
        ...
        tree_y_pred = np.array(predict).reshape(-1,1)

        col1 = ann_y_pred
        col2 = tree_y_pred
        col3 = y_test.reshape(-1,1)
        col1 = pd.DataFrame(col1).astype(int)
        col2 = pd.DataFrame(col2).astype(int)
        col3 = pd.DataFrame(col3).astype(int)
        ...
        params = {
            ...
            'name' : 'corrected_combined_ann_trees-02-07-2019',
            '0,0' : float(cm[0][0]),
            '0,1' : float(cm[0][1]),
            '1,0' : float(cm[1][0]),
            '1,1' : float(cm[1][1]),
